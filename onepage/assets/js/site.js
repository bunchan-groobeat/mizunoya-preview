...
(function () {
  ...
  function initReveal() {
    ...
    /* ★既定は「毎回」＝画面から外れると元に戻し、再び入るともう一度動く（社長決定 2026-08-13）。
         <html data-reveal-once> を付けたときだけ「1回だけ」に切り替わる。
       ★unobserve は使わない。使うと、既に一度出た要素が二度と動かなくなる。
         監視対象はセクション数個なので、監視を続けても負荷は問題にならない。
       ★判定はコールバックの中で毎回読む＝実行中に切り替えても効く。 */
    var io = new IntersectionObserver(function (entries) {
      var once = document.documentElement.hasAttribute('data-reveal-once');
      entries.forEach(function (e) {
        if (e.isIntersecting) e.target.classList.add('is-revealed');
        else if (!once) e.target.classList.remove('is-revealed');
      });
    }, {
